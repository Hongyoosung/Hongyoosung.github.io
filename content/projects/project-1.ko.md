...
218-dim 관측 벡터에서 각 엔티티 슬롯은 선형 인코더(`nn.Linear`)를 통해 먼저 독립적으로 임베딩됩니다. 이 상태에서 Self Token 하나가 Cross-Attention의 Query로 엔티티 집합을 조회하면, Attention weight는 "Self 관점에서 각 엔티티가 얼마나 참고할 만한가"를 나타냅니다. 하지만 Key/Value로 들어가는 엔티티 토큰 자체가 서로를 참조한 적이 없기 때문에, **엔티티 간 상대적 관계**(밀집도, 협공 패턴, 동일 거점 중복)는 Cross-Attention weight에 직접 반영되기 어렵습니다. Cross-Attention은 이미 주어진 엔티티 표현을 선택적으로 집약할 수는 있지만, 엔티티들 사이의 관계 표현을 새로 만들어 주지는 못했습니다.

---

<font size="4">**Goal**</font>

Cross-Attention이 "Self 기준의 정보 선택"을 수행하기 전에, 각 엔티티 토큰이 같은 그룹 안의 다른 토큰을 먼저 참조하도록 만들어 관계 정보를 포함한 표현으로 변환합니다. 이를 통해 보상 함수를 더 복잡하게 늘리지 않고도, 정책 입력 단계에서 중복 배치·밀집·협공 같은 공간 맥락을 행동 결정에 사용할 수 있게 합니다.
...
각 엔티티 그룹(아군 / 적 / 거점)에 대해, Cross-Attention 이전에 **Self-Attention 레이어**를 삽입하여 엔티티들이 서로를 참조하도록 했습니다. Self-Attention을 거친 엔티티 토큰은 "이 아군은 이미 다른 아군과 같은 거점 근처에 있다", "이 적들은 같은 거점 방향으로 모여 있다"와 같은 문맥 정보를 포함하게 됩니다. 이후 Cross-Attention에서 Self Token은 단순한 개별 엔티티가 아니라 이 **문맥화된** 엔티티 정보를 집약하므로, 최종 EQS 가중치가 개별 중요도뿐 아니라 팀 배치와 공간 패턴까지 반영할 수 있습니다.
