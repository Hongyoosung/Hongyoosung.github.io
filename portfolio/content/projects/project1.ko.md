...
언리얼엔진 5를 지원하는 강화학습 플러그인인 Schola(gRPC)와 Ray RLlib 사이에 재사용 가능한 RL-EQS 미들 레이어를 구현하고, 플러그인의 결함을 수정한 PR이 Schola main / v2.1.1 기준으로 머지되었습니다.

##### 학습한 점

Unreal 환경을 RL 학습 루프에 연결할 때는 액션, 관측, 보상뿐 아니라 agent별 종료 상태까지 step contract로 명확히 관리해야 한다는 점을 배웠습니다.

#### Attention 기반 관계 인식

##### 주요 내용

에이전트가 개별 개체 정보만 보는 한계를 넘어, 아군·적·거점 사이의 공간 관계를 관측 인코더에서 읽도록 개선했습니다. 동일 조건(66 iteration)에서 Self-Attention 유무를 비교한 A/B 실험 결과, Self-Attention 적용 시 objective coverage가 **0.318 → 0.454 (+43%)** 로 상승했고, 팀 전원이 한 거점에 몰리는 실패 패턴은 **46.5% → 16%** 로 감소했으며, 스크립트 AI 상대 승률은 **0.125 → 0.292(2.3배)** 로 향상되었습니다.

##### 학습한 점

어텐션은 협동 행동을 자동으로 완성하는 장치가 아니라, 정책이 행동을 결정하기 전에 팀 배치와 공간 맥락을 더 정확히 해석하도록 돕는 인코더 설계임을 확인했습니다.

#### 커리큘럼 학습

##### 주요 내용

Scripted AI를 Basic, Standard, Aggressive 3단계 티어로 구성하고 승률 25%, 40% 임계값 기반 승격 구조를 적용했습니다. 최종적으로 2.5M 타임스텝 학습 후 Scripted AI 상대 승률이 0%에서 약 70% 수준까지 상승했습니다.

##### 학습한 점

초기부터 어려운 상대나 self-play를 적용하면 학습 신호가 불안정해지므로, 난이도를 점진적으로 올리는 커리큘럼이 게임 규칙 학습과 정책 안정화에 중요하다는 점을 확인했습니다.

#### MAPPO 멀티 에이전트 학습

##### 주요 내용

Strike, Vanguard, Support 역할별 독립 actor와 shared centralized critic을 사용하는 MAPPO 학습 환경을 구성하고 체크포인트간 승률 비교 스크립트를 구현하였습니다.

##### 학습한 점

멀티 에이전트 환경에서는 단일 보상 스칼라만으로 협동을 설명하기 어렵고, 역할별 정책, 중앙 critic, 체크포인트 평가 루프를 함께 설계해야 학습 품질을 추적할 수 있다는 점을 배웠습니다.

#### GAS 기반 전투 환경

##### 주요 내용

UE5 Gameplay Ability System을 기반으로 Strike의 원거리 전투, Vanguard의 근접 교전, Support의 치유 행동을 구성하고, 접근, 거점, 전투 보상을 매 스텝 계산하는 전투-RL 보상 파이프라인을 구현했습니다.

##### 학습한 점

강화학습 환경의 보상은 추상 수식만으로 설계하기보다 실제 전투 시스템의 데미지, 치유, 위치, 역할 제약과 연결해야 에이전트 행동이 게임플레이 의도와 맞아진다는 점을 배웠습니다.

#### AWS 클라우드 분산 학습

##### 주요 내용

UE5 Linux 빌드를 S3에 적재하고 Docker 기반 학습 스크립트를 EC2에서 실행하는 클라우드 병렬 학습 파이프라인을 구축했습니다.

##### 학습한 점

Unreal 기반 RL은 로컬 단일 인스턴스만으로 반복 속도가 부족하므로, 빌드 배포, 체크포인트 동기화, 원격 평가를 포함한 인프라 설계가 중요하다는 점을 배웠습니다.

---

### 시스템 아키텍처 (System Architecture)

<figure class="project-media"><img src="/images/project1/archi.png" alt="Fig 1. 시스템 아키텍처 및 계층적 포지셔닝 워크플로우" loading="lazy"><figcaption>Fig 1. 시스템 아키텍처 및 계층적 포지셔닝 워크플로우</figcaption></figure>

* * *

### 기술 스택 (Tech Stack)

<figure class="project-media"><img src="/images/project1/teckstack.png" alt="" loading="lazy"></figure>

* * *

### 훈련 환경 (Training Environment)

**5 vs 5 팀 기반 거점 점령전**으로, 맵 상에 5개의 거점(Capture Point)이 배치됩니다. 더 많은 거점을 점령·유지하여 목표 점수를 먼저 달성하는 팀이 승리합니다.


#### 클래스 역할 설계

각 에이전트는 매 에피소드마다 **Strike / Vanguard / Support** 클래스를 부여받습니다. 클래스마다 요구되는 포지셔닝 전략이 근본적으로 다르므로, 각 클래스에 **독립된 정책 네트워크**를 할당하고, 역할에 특화된 보상 함수를 적용했습니다.
...
<img src="/formula/project1/final_reward_formula.svg" class="project-math-svg" alt="팀 보상 믹싱 수식">
...
팀 평균은 자신을 제외한 동일 팀 에이전트들의 `LastIndividualStepReward` 평균값이며, 팀원이 1명일 경우 믹싱은 자동으로 비활성화됩니다. 이 계산은 `DERewardSubsystem.cpp`의 `CalculateStepReward()` 함수에서 역할별 보상 산출 직후 매 스텝 적용됩니다.
...
<figure class="project-media"><img src="/images/project1/rewardpipeline.png" alt="Fig 2. 보상 파이프라인" loading="lazy"><figcaption>Fig 2. 보상 파이프라인</figcaption></figure>
...
4.  **Dual Critic 평가:** `EntityCentricRLlibModel`의 value function이 학습 가능한 혼합 계수(α, sigmoid 초기값 0.5)를 사용하여 V\_local(226-dim 에이전트 관찰 기반 attention critic)과 V\_central(71-dim 글로벌 팀 상태 기반 MLP critic)을 결합합니다. 세 역할 정책이 centralized critic을 공유합니다.
...
<figure class="project-media"><img src="/images/project1/pluginarchi.png" alt="Fig 3. 플러그인 계층 구조. 붉은 박스가 실제 구현한 부분입니다" loading="lazy"><figcaption>Fig 3. 플러그인 계층 구조. 붉은 박스가 실제 구현한 부분입니다</figcaption></figure>

<figure class="project-media"><img src="/images/project1/InferenceLoop.png" alt="Fig 4. 학습/추론 런타임 데이터 플로우" loading="lazy"><figcaption>Fig 4. 학습/추론 런타임 데이터 플로우</figcaption></figure>
...
<div class="project-media-grid"><figure class="project-media"><img src="/images/project1/eqs1.png" alt="Fig 5. DynamicEQSExecutor 컴포넌트" loading="lazy"><figcaption>Fig 5. DynamicEQSExecutor 컴포넌트</figcaption></figure><figure class="project-media"><img src="/images/project1/eqs2.png" alt="Fig 6. EQS 에셋 구성" loading="lazy"><figcaption>Fig 6. EQS 에셋 구성</figcaption></figure></div>
...
<figure class="project-media"><img src="/images/project1/eval.png" alt="Fig 9. 모델 평가 결과" loading="lazy"><figcaption>Fig 9. 모델 평가 결과</figcaption></figure>
...
<img src="/formula/project1/agent_obs_formula.svg" class="project-math-svg" alt="Dual Value Estimation 수식">
...
로컬 단일 UE5 인스턴스의 한계를 넘어 클라우드 병렬 환경으로 확장하기 위해 AWS 기반 분산 학습 파이프라인을 구축했습니다. UE5를 Linux 바이너리로 패키징하고 S3에 적재, 학습 스크립트를 Docker 이미지로 EC2 인스턴스에서 실행하여 병렬 학습을 수행합니다.

* * *

#### 전체 인프라 구성

<figure class="project-media"><img src="/images/project1/aws_archi.png" alt="Fig 10. AWS 병렬 학습 인프라 전체 구성도" loading="lazy"><figcaption>Fig 10. AWS 병렬 학습 인프라 전체 구성도</figcaption></figure>
...
| **EC2 Ray 클러스터** | Ray head + env\_runners 4개 동시 실행, GPU로 PPO 학습 |
...
Head 노드(GPU)는 정책 gradient 업데이트를, Worker 노드(CPU Spot)는 UE5 헤드리스 인스턴스 구동 및 롤아웃 수집을 전담합니다. 롤아웃 수집은 중단·재시작이 가능한 stateless 작업이기 때문에 Worker는 Spot 인스턴스로 구성하여 비용을 절감했습니다.
...
`ray down` 시 모든 인스턴스가 즉시 종료되며, S3 체크포인트는 유지되어 재기동 시 이어서 학습할 수 있습니다.

<div class="project-media-grid"><figure class="project-media"><img src="/images/project1/aws_ec2.png" alt="Fig 11. AWS EC2 인스턴스" loading="lazy"><figcaption>Fig 11. AWS EC2 인스턴스</figcaption></figure><figure class="project-media"><img src="/images/project1/aws_ps.png" alt="Fig 12. ps aux 커맨드로 확인한 4개의 PID(UE5 프로세스)" loading="lazy"><figcaption>Fig 12. ps aux 커맨드로 확인한 4개의 PID(UE5 프로세스)</figcaption></figure></div>

<div class="project-media-grid"><figure class="project-media"><img src="/images/project1/aws_s3.png" alt="Fig 13. AWS S3" loading="lazy"><figcaption>Fig 13. AWS S3</figcaption></figure><figure class="project-media"><img src="/images/project1/aws_log.png" alt="Fig 14. CloudWatch Log" loading="lazy"><figcaption>Fig 14. CloudWatch Log</figcaption></figure></div>

<div class="project-media-grid"><figure class="project-media"><img src="/images/project1/wan_spec.png" alt="Fig 15. WAN DB Spec" loading="lazy"><figcaption>Fig 15. WAN DB Spec</figcaption></figure><figure class="project-media"><img src="/images/project1/wan_log.png" alt="Fig 16. WAN DB Log" loading="lazy"><figcaption>Fig 16. WAN DB Log</figcaption></figure></div>
...
<figure class="project-media"><img src="/images/project1/Schola-ScholaArchitecture.png" alt="Fig 17a. Schola 플러그인 통신 구조" loading="lazy"><figcaption>Fig 17a. Schola 플러그인 통신 구조</figcaption></figure>
...
<figure class="project-media"><img src="/images/project1/Schola-DeadAgent(Before).png" alt="Fig 17b. 수정 전 시차 사망 처리 문제" loading="lazy"><figcaption>Fig 17b. 수정 전 시차 사망 처리 문제</figcaption></figure>
...
<figure class="project-media"><img src="/images/project1/Schola-DeadAgent(After).png" alt="Fig 17c. dead agent 필터링 후 안정화된 스텝 동기화" loading="lazy"><figcaption>Fig 17c. dead agent 필터링 후 안정화된 스텝 동기화</figcaption></figure>
...
<div class="project-media-grid"><figure class="project-media"><img src="/images/project1/pr1.png" alt="Fig 18. 머지된 Schola Pull Request" loading="lazy"><figcaption>Fig 18. 머지된 Schola Pull Request</figcaption></figure><figure class="project-media"><img src="/images/project1/pr2.png" alt="Fig 19. Schola PR 변경 사항 및 테스트" loading="lazy"><figcaption>Fig 19. Schola PR 변경 사항 및 테스트</figcaption></figure></div>

* * *

#### Problem 2: 엔티티 간 관계 정보 손실

<figure class="project-media"><img src="/images/project1/problem2.png" alt="Fig 20. 엔티티 관계 정보 손실과 어텐션 도입을 통한 해결" loading="lazy"><figcaption>Fig 20. 엔티티 관계 정보 손실과 어텐션 도입을 통한 해결</figcaption></figure>

초기에는 MAPPO 기반의 팀 협동 플레이 보상을 설계하여 협동성을 유도하려 했습니다. 그러나 Strike / Support / Vanguard 3가지 역할 간의 협동을 하나의 스칼라 보상으로 정의하는 것이 모호했고, 보상 해킹(Reward Hacking)이 빈번히 발생했습니다.

따라서 협동성을 직접 보상 항으로 강제하기보다, 정책이 관측 정보를 읽는 구조를 개선하는 방향으로 전환했습니다. 먼저 Cross-Attention을 사용해 Self Token(현재 에이전트 상태)이 아군·적·거점 토큰 중 행동 결정에 필요한 정보를 가중합하도록 했습니다. 이 가중치는 보상 그 자체가 아니라, 7차원 EQS 가중치 액션을 산출하기 직전의 정보 선택 결과입니다. 즉 "현재 나에게 어떤 엔티티가 중요한가"는 반영할 수 있었지만, "아군 2명이 이미 같은 거점 주변에 있다" 또는 "적 2명이 같은 거점에 집결 중이다"처럼 엔티티끼리 이루는 공간 패턴은 충분히 표현하지 못했습니다.

이 한계 때문에 이미 점령된 friendly zone에 머무르는 패널티나 non-friendly objective 접근 보상이 있어도, 정책은 여러 아군이 같은 거점에 몰려 있다는 맥락을 안정적으로 구분하지 못했습니다. 결과적으로 각 에이전트가 같은 거점을 개별적으로 중요하다고 판단해 중복 배치되는 현상이 발생했습니다.

* * *

#### Cause

218-dim 관측 벡터에서 각 엔티티 슬롯은 선형 인코더(`nn.Linear`)를 통해 먼저 독립적으로 임베딩됩니다. 이 상태에서 Self Token 하나가 Cross-Attention의 Query로 엔티티 집합을 조회하면, Attention weight는 "Self 관점에서 각 엔티티가 얼마나 참고할 만한가"를 나타냅니다.

하지만 Key/Value로 들어가는 엔티티 토큰 자체가 서로를 참조한 적이 없기 때문에, **엔티티 간 상대적 관계**(밀집도, 협공 패턴, 동일 거점 중복)는 Cross-Attention weight에 직접 반영되기 어렵습니다. Cross-Attention은 이미 주어진 엔티티 표현을 선택적으로 집약할 수는 있지만, 엔티티들 사이의 관계 표현을 새로 만들어 주지는 못했습니다.

* * *

#### Goal

보상 함수를 더 복잡하게 늘리지 않고도, 정책 입력 단계에서 중복 배치·밀집·협공 같은 공간 맥락을 행동 결정에 사용할 수 있게 하는 것.
...
각 엔티티 그룹(아군 / 적 / 거점)에 대해, Cross-Attention 이전에 **Self-Attention 레이어**를 삽입하여 엔티티들이 서로를 참조하도록 했습니다. Self-Attention을 거친 엔티티 토큰은 "이 아군은 이미 다른 아군과 같은 거점 근처에 있다", "이 적들은 같은 거점 방향으로 모여 있다"와 같은 문맥 정보를 포함하게 됩니다.

이후 Cross-Attention에서 Self Token은 단순한 개별 엔티티가 아니라 이 **문맥화된** 엔티티 정보를 집약하므로, 최종 EQS 가중치가 개별 중요도뿐 아니라 팀 배치와 공간 패턴까지 반영할 수 있습니다.
...
이 문제는 단순한 설계 가설이 아니라 실제 Unreal 평가 로그에서 확인되었습니다. 최종 Self+Cross Attention 체크포인트를 24 episode, 15,715 team-step 동안 평가한 결과, 실현 위치 기준 objective coverage는 **0.454**, 평균 unique objective는 **2.27 / 5**, 가장 붐비는 거점의 평균 군집 크기는 **3.36명**이었습니다. 5명과 5개 거점 구조에서는 “두 명 이상이 같은 거점을 선택했는가”라는 이진 duplicate rate가 거의 항상 1.0으로 포화되어, coverage와 max-cluster size를 핵심 지표로 사용했습니다.

실증 결과는 위의 **"Attention 패턴 실증 분석"** 섹션에서 확인할 수 있습니다. 특히 동일 조건(66 iteration)에서 Self-Attention을 끈 모델을 추가로 학습해 비교한 통제된 A/B ablation으로 기여를 직접 확인했습니다: Self-Attention 적용 시 objective coverage가 **0.318 → 0.454 (+43%)**, '전원이 한 거점에 몰리는' 실패 패턴이 **46.5% → 16%**로 감소했고, 승률은 **0.125 → 0.292(2.3배)**로 향상되었습니다(보상은 거의 동일)—인코더 개선이 관계 인식 자체를 넘어 팀 분산을 실제로 개선했음을 보여줍니다.

* * *

#### Problem 3: 스텝 속도와 에이전트 이동 간의 타이밍 불일치

에이전트가 이전 스텝에서 결정된 목적지에 도달하기 전에 새로운 스텝이 실행되어 이동이 취소되는 문제가 발생하여 학습의 품질이 크게 훼손되었습니다.
...
Self-Attention은 보상 크기를 단순히 키운 것이 아니라, 에이전트가 행동을 결정하기 전에 아군·적·거점 사이의 공간 관계를 읽도록 만드는 관측 인코더 역할을 했습니다. 동일 조건의 A/B 평가에서 Self-Attention을 추가한 모델은 objective coverage를 **0.318 → 0.454(+43%)** 로 높였고, 팀 전원이 한 거점에 몰리는 실패 패턴을 **46.5% → 16.0%** 로 줄였으며, 스크립트 AI 상대 승률을 **0.125 → 0.292(2.3배)** 로 개선했습니다. 평균 에피소드 보상은 거의 동일했기 때문에, 개선의 핵심은 보상 총량보다 **팀이 거점에 분산되는 방식** 에 있었습니다.

#### 검증 설계

분석은 세 축으로 구성했습니다. 먼저 Python synthetic probe로 Self-Attention과 Cross-Attention이 대형 변화에 반응하는지 확인했습니다. 다음으로 Unreal short evaluation에서 실제 팀 배치가 거점에 어떻게 분포되는지 측정했습니다. 마지막으로 Self-Attention을 제거한 Cross-only 모델을 동일 조건으로 학습하고 같은 평가 프로토콜로 비교하여, Self-Attention 블록의 기여를 분리했습니다.

| 검증 항목 | 측정 결과 | 해석 |
| --- | --- | --- |
| 패딩 마스크 | 패딩 슬롯 attention 최대값 **0.000** | 유효하지 않은 엔티티 슬롯이 attention 계산에 누수되지 않았습니다. |
| 대형 변화 민감도 | 아군 Self-Attention DeltaMax: Vanguard 0.1086, Strike 0.0945, Support 0.0582. 적군 DeltaMax: Strike 0.1165 | Vanguard는 아군 대형 변화에, Strike는 적군 집결 변화에 더 민감하게 반응했습니다. |
| Self → Cross 전달 | 전체 Self-Cross consistency 0.639, Vanguard 0.917, Strike ally Cross TV shift 0.1347 | Self-Attention에서 형성된 문맥 변화가 행동 직전 Cross-Attention 분포까지 전달되었습니다. |
| 중복 회피 proxy | Strike의 crowded-objective avoidance proxy에서 Self-Attention gain **+0.0178** | 중복 회피 신호는 Strike에서 가장 뚜렷했으며, 모든 역할에 동일하게 나타나는 보편 효과는 아니었습니다. |
| Unreal 행동 평가 | coverage 0.454, 평균 unique objective 2.27/5, 평균 max-cluster size 3.36명 | 실제 플레이에서는 군집 문제가 일부 남았지만, A/B ablation에서 Self-Attention이 군집을 의미 있게 줄였습니다. |
...
<figure class="project-media"><img src="/images/project1/unreal_eval_behavior_metrics.png" alt="Unreal evaluation behaviour metrics" loading="lazy"><figcaption>Fig 26. Unreal 행동 평가 지표</figcaption></figure>

<figure class="project-media"><img src="/images/project1/unreal_eval_clustering_profile.png" alt="Unreal objective clustering profile" loading="lazy"><figcaption>Fig 27. Objective coverage 및 clustering profile</figcaption></figure>

#### A/B Ablation: Self-Attention 유무 비교

Self-Attention을 끈 모델(`USE_SELF_ATTN=0`, Cross-Attention 전용 변형)을 최종 Self+Cross 모델과 같은 맵, 같은 스크립트 AI 난이도, 같은 MAPPO 설정, 같은 **66 iteration / 약 136만 agent-step** 조건으로 학습했습니다. 이후 두 체크포인트를 동일한 24-episode·8-environment 프로토콜로 평가했습니다. 두 모델의 유일한 구조적 차이는 Self-Attention 블록이므로, 아래 차이는 관측 인코더의 관계 인식 효과로 해석할 수 있습니다.

| 지표 (실현 위치/nearest 기준) | Cross-only | Self + Cross | 변화 |
| --- | --- | --- | --- |
| Objective coverage | 0.318 | **0.454** | **+0.136 (+43%)** |
| 평균 unique objective (5개 중) | 1.59 | **2.27** | **+0.68** |
| 최대 군집 크기 (낮을수록 좋음) | 4.10 | **3.36** | **−0.74명** |
| 전원(5명)이 한 거점에 몰린 스텝 비율 | 46.5% | **16.0%** | **−30.5%p** |
| Coverage 추이 (초반 → 후반) | 0.28 → 0.32 (정체) | 0.36 → 0.52 (상승) | 에피소드 진행 중 분산 개선 |
| 스크립트 AI 상대 승률 | 0.125 (3/24) | **0.292 (7/24)** | **+0.167 (2.3배)** |
| 평균 에피소드 보상 | 152.7 | 148.9 | −3.8 (거의 동일) |

<figure class="project-media"><img src="/images/project1/attn_ab_self_attn_effect.png" alt="학습된 A/B ablation: Cross-only vs Self+Cross — Self-Attention 효과" loading="lazy"><figcaption>Fig 28. 학습된 A/B ablation: Self-Attention 추가 시 objective coverage 상승, 군집 감소, 승률 2배 이상.</figcaption></figure>

<figure class="project-media"><img src="/images/project1/attn_ab_clustering_comparison.png" alt="A/B 군집 비교: unique-objective 분포 및 에피소드 진행에 따른 coverage" loading="lazy"><figcaption>Fig 29. Self-Attention 적용 전후의 unique-objective 분포와 에피소드 진행에 따른 coverage 변화.</figcaption></figure>

#### Attention Weight 시각 분석

학습된 체크포인트에서 attention weight를 직접 추출하여 Self-Attention → Cross-Attention 파이프라인이 설계 의도대로 연결되는지 확인했습니다. 동일한 정책에 두 시나리오를 입력했습니다. **Clustered**는 아군 4명이 동일 거점 반경 내에 밀집한 경우이고, **Spread**는 아군 4명이 맵의 서로 다른 코너에 분산된 경우입니다. 모든 역할과 시나리오에서 패딩 슬롯 4–7의 attention weight는 0.00으로 억제되었습니다.

#### Self-Attention: 엔티티 간 공간 관계 포착

Self-Attention은 Cross-Attention 이전에 엔티티 토큰들이 서로를 참조하도록 만드는 단계입니다. 이 단계의 핵심은 개별 아군·적·거점 슬롯을 독립적으로 읽는 것이 아니라, 밀집도와 대형 변화 같은 **엔티티 간 관계**를 토큰 표현에 반영하는 것입니다.

| 역할 | 대형 민감도 | 주요 관찰 |
| --- | --- | --- |
| **Strike** | 중간 (Δmax ≈ 0.19) | Clustered에서는 4개 슬롯을 비교적 균등하게 참조하고, Spread에서는 Assault 슬롯 1에 집중했습니다. |
| **Support** | 낮음 (Δmax ≈ 0.06) | 대형과 무관하게 전 슬롯을 균등하게 유지했습니다. 팀 전체를 안정적으로 모니터링해야 하는 회복 역할과 일치합니다. |
| **Vanguard** | 높음 (Δmax ≈ 0.29) | Clustered에서는 슬롯 3, Spread에서는 슬롯 1로 초점이 이동했습니다. 전선 앵커 역할이 팀 배치 변화에 가장 민감하게 반응했습니다. |
...
#### Cross-Attention: 행동 직전 정보 집약

Cross-Attention은 Self Token이 Query가 되어 아군·적·거점 토큰을 조회하는 단계입니다. 이 결과는 EQS 가중치, 즉 실제 행동 결정으로 이어지므로 에이전트가 행동 직전에 어떤 정보를 참고했는지 보여줍니다. 적 슬롯은 전 역할에서 거의 균등한 위협으로 처리되었고, 거점 슬롯은 역할별로 다른 우선순위를 보였습니다. 아군 슬롯에서는 Self-Attention에서 형성된 초점이 Cross-Attention에서도 유지되는지 확인했습니다.
...
가장 명확한 사례는 Vanguard입니다. Clustered에서는 Self-Attention과 Cross-Attention이 모두 슬롯 3을 강하게 참조했고, Spread에서는 두 단계 모두 슬롯 1로 초점이 이동했습니다. 이는 Self-Attention이 만든 문맥화된 표현을 Cross-Attention이 행동 직전 정보 집약 단계에서 일관되게 활용했음을 보여줍니다.

#### 해석 범위

이 결과는 Self-Attention이 관측 인코더에 공간 관계를 읽는 능력을 제공한다는 encoder-level evidence입니다. 역할 자체는 역할별 정책과 보상 함수의 영향을 함께 받았으며, attention만으로 완전한 협동이나 자발적 역할 분화가 발생했다고 해석해서는 안 됩니다. 또한 현재 결과는 모델당 단일 평가 세션의 점 추정치이므로, 다중 시드 검증은 후속 과제로 남습니다.
...
| 지표 | 값 | 의미 |
| --- | --- | --- |
| **episode\_mean** | 0 → 40, 17, 48 수렴 |
| **vf/explained\_var** | 0.8 이상 | Critic이 미래 보상의 80% 이상을 설명 — 높은 상태 가치 예측력 |
