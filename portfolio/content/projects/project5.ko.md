...
**AI 에이전트를 활용한 가상 공장 시뮬레이션**은 산업 현장의 운영 전략을 Unreal Engine 5 기반 디지털 트윈에서 검증하기 위한 **시뮬레이션 제어 플랫폼**입니다. 사용자는 "AGV를 몇 대 투입할 것인가", "병목률 5% 이하의 최적 AGV 대수를 찾아줘"와 같은 자연어 요청을 입력하고, 시스템은 LangGraph 기반 에이전트 라우팅, 로컬 LLM Tool Calling, Validation Layer, UE5 Command Proxy를 거쳐 실제 시뮬레이션 실행과 KPI 리포트까지 자동화합니다. 여기에 다국어 Vector RAG와 typed graph를 결합해 운영 절차, 실시간 설비 상태, 과거 run KPI를 출처가 추적 가능한 답변으로 연결했습니다.
...
"병목률 5% 이하의 최적 AGV 대수를 찾아줘"와 같은 목표 탐색형 요청은 단일 Tool Call로 끝나지 않습니다. 이 시스템은 최적화 요청을 사전 판정한 뒤 목표 지표를 파싱하고, UE5 상태에서 탐색 가능한 AGV 상한을 조회한 뒤 **observe - judge - decide - re-run** 폐루프를 실행합니다.
...
이 플랫폼은 시뮬레이션 실행 후에도 사용자의 후속 요청을 같은 대화 흐름 안에서 처리합니다. "현재 상태 알려줘"는 실시간 텔레메트리 요약으로, "시뮬레이션 종료해줘"는 종료 명령과 상태 확인으로, "결과 보고해줘"는 KPI 기반 자연어 리포트로 연결됩니다.
